...
public class DialogCompaniesTableController implements BaseController {
    ...
    @FXML
    private TextField textIncomeTax; //расходы по налогу на прибыль
    @FXML
    private TextField textFinancialIncome;
    @FXML
    private TextField textDepreciation; //амортизация
    @FXML
    ...
    public void save(ActionEvent actionEvent) {
        ...
        try {
            ...
            if (succor == 0 || incomeTax <= 0 || financialIncome <= 0 || depreciation == 0) {
                ShowAlert.showErrorAlert("Денежные поля не могут равняться нулю и быть отрицательными!");
